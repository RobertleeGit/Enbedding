...
static key_press_event_t press_event = {0};
...
{
    // Trace the interrupt delay
    HAL_GPIO_WritePin(IRQ_TRACE_GPIO_Port, IRQ_TRACE_Pin, GPIO_PIN_SET);
    
    if (GPIO_Pin == Key_Pin)
    {
        press_event.trigger_tick = HAL_GetTick();   // 获取当前系统 tick（单位 ms）
        
        // 读取当前电平确定方向
        if (HAL_GPIO_ReadPin(Key_GPIO_Port, Key_Pin) == GPIO_PIN_RESET)
            press_event.direction = LEVEL_FALLING;
        else
            press_event.direction = LEVEL_RISING;

        key_press_event_t *p_event = &press_event;
        // 从中断中发送到队列（使用中断安全函数）
        BaseType_t xHigherPriorityTaskWoken = pdFALSE;
        xQueueSendFromISR(key_irq_Queue, &p_event, &xHigherPriorityTaskWoken);
...
void Key_Task(void *argument)
{
    ...
    key_press_event_t *press_event = NULL;
