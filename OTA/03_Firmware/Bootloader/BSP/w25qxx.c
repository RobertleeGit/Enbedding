...
static void W25Qx_Reset(void)
{
	...
	SPI_FLASH_Transmit(cmd, 2, W25QXXXX_TIMEOUT_VALUE);
	W25Qx_Disable();
}
...
static uint8_t W25Qx_GetStatus(void)
{
	...
	SPI_FLASH_Transmit(cmd, 1, W25QXXXX_TIMEOUT_VALUE);
	/* Reception of the data */
	SPI_FLASH_Receive(&status, 1, W25QXXXX_TIMEOUT_VALUE);
	W25Qx_Disable();
 ...
 * @note  	Device ID to Chip Model:
 *  	   	0XEF13: Chip Model W25Q80
 *  		0XEF14: Chip Model W25Q16
 *  		0XEF15: Chip Model W25Q32
 *  		0XEF16: Chip Model W25Q64
 * @retval None
 */
void W25Qx_Read_ID(uint16_t *ID)
{
	...
	W25Qx_Enable();
	/* Send the read ID command */
	SPI_FLASH_Transmit(cmd, 4, W25QXXXX_TIMEOUT_VALUE);
	/* Reception of the data */
	SPI_FLASH_Receive(idt, 2, W25QXXXX_TIMEOUT_VALUE);
...
uint8_t W25Qx_Read(uint8_t *pData, uint32_t ReadAddr, uint32_t Size)
{
	...
	SPI_FLASH_Transmit(cmd, 4, W25QXXXX_TIMEOUT_VALUE);
	/* Reception of the data */
	if (SPI_FLASH_Receive(pData, Size, W25QXXXX_TIMEOUT_VALUE) != SPI_FLASH_OK)
	{
		return W25Qx_ERROR;
	}
	W25Qx_Disable();
...
uint8_t W25Qx_Write(uint8_t *pData, uint32_t WriteAddr, uint32_t Size)
{
	...
	do
	{
		...
		W25Qx_Enable();
		/* Send the command */
		if (SPI_FLASH_Transmit(cmd, 4, W25QXXXX_TIMEOUT_VALUE) != SPI_FLASH_OK)
		{
			return W25Qx_ERROR;
		}

		/* Transmission of the data */
		if (SPI_FLASH_Transmit(pData, current_size, W25QXXXX_TIMEOUT_VALUE) != SPI_FLASH_OK)
		{
			return W25Qx_ERROR;
		}
		W25Qx_Disable();
