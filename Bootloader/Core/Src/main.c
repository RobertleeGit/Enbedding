...
/* Private typedef -----------------------------------------------------------*/
/* USER CODE BEGIN PTD */
typedef void (*pFunction) (void);		// 声明一个函数指针类型
/* USER CODE END PTD */

/* Private define ------------------------------------------------------------*/
/* USER CODE BEGIN PD */
// APP 起始地址
#define FLASH_APP_ADDR			0x08019000
/* USER CODE END PD */
...
/* USER CODE BEGIN PV */
static pFunction jump_to_Application;	// 声明一个函数指针变量
/* USER CODE END PV */
...
void JumpToAPP(void)
{
	...
	{
		// 获取 PC 指针的地址（程序入口地址）
		jumpAddr = *(__IO uint32_t *) (FLASH_APP_ADDR + 4);
		// 将函数指针指向该地址
